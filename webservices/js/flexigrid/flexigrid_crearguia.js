 ...
 * Y MODIFICADO UN POCO POR MI MEZCLANDO LA VERSIONN ULTIMA CON LA DEL EJEMPLO
 */
...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		p = $.extend({
			 ...
			 title: false,
			 pagestat: 'Mostrando desde {from} a {to} de {total} items',
			 pagetext: 'Página',
			 outof: 'de',
			 findtext: 'Find',
		...
		var g = {
			...
			populate: function () { //get latest data
				...
				if(p.p_adicionales){
					param2_name = [p.p_adicionales[0].nombre];
					param2_values = [$(p.p_adicionales[0].campo).val()];
					param.push({name : param2_name[0], value : param2_values[0]});
					for (i=1;i<=p.p_adicionales.length -1;i++){
						param2_name.push(p.p_adicionales[i].nombre);
						param2_values.push($(p.p_adicionales[i].campo).val());
						param.push({name : param2_name[i], value : param2_values[i]});
					}
				}
		...
		if (p.searchitems)
			{
				...
				$(g.sDiv).append("<div class='sDiv2'>Búsqueda <input autocomplete='off' type='text' size='30' name='q' class='qsbox' /> <select name='qtype'>"+sopt+"</select></div>");

				$('input[name=q],select[name=qtype]',g.sDiv).keyup(function(e){ g.doSearch() });
				//$('input[value=Reiniciar]',g.sDiv).click(function(){$('input[name=q]',g.sDiv).val(''); p.query = ''; g.doSearch(); });
				$(g.bDiv).after(g.sDiv);				
